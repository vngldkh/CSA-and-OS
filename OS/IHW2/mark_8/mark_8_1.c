...
void init() {
    ...
    int pid = shm_open("/p-sh-m", O_CREAT | O_RDWR, 0666);
    ftruncate(pid, nprog * sizeof(pid_t));
    processes = (pid_t*) mmap(0, nprog * sizeof(pid_t), PROT_READ | PROT_WRITE, MAP_SHARED, pid, 0);
...
void destroy() {
    ...
    munmap(processes, nprog * sizeof(pid_t));
    shm_unlink("/p-sh-m");
}
...
void review(int author, int program_no) {
