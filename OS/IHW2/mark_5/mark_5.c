...
int main() {
    ...
    int semid = shm_open("/semaphores", O_CREAT | O_RDWR, 0666);
    ftruncate(semid, nprog * sizeof(sem_t));
    sem = mmap(0, nprog * sizeof(sem_t), PROT_READ | PROT_WRITE, MAP_SHARED, semid, 0);
    for (int i = 0; i < nprog; ++i) {
        sem_init(&sem[i], 1, 1);
    }
