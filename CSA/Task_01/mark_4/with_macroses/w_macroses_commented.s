...
.L2:
	...
	lea	rax, -100[rbp]
	mov	rsi, rax
	# Первый аргумент - строка "%d", кладем в rdi
	lea	rax, .LC1[rip]
	mov	rdi, rax
	mov	eax, 0
...
.L11:
	...
	mov	esi, eax
	# Сама строка - первый аргумент, записываем в di
	lea	rax, .LC2[rip]
	mov	rdi, rax
	mov	eax, 0
	call	printf@PLT
	...
	# Адрес на A[i] - второй аргумент, кладём в si (смещение + i * 8)
	lea	rdx, 0[0+rax*8]
	mov	rax, QWORD PTR -80[rbp]
	add	rax, rdx
	mov	rsi, rax
	# Строка "%lf" - первый аргумент, кладём в di
	lea	rax, .LC3[rip]
	mov	rdi, rax
	mov	eax, 0
	call	__isoc99_scanf@PLT
...
.L10:
	...
	# Если i < size, входим в цикл (.L11)
	jl	.L11

	# Иначе инициализируем i для следующего цикла
	mov	DWORD PTR -92[rbp], 0

	# Переходим в .L12 (Проверка условия цикла)
	jmp	.L12
.L13:
...
.L12:
	...
	# Если i < size, входим в тело цикла (.L13)
	jl	.L13

	# Возвращаем stack pointer в изначальную позицию, проверяем, что всё сошлось
	mov	eax, 0
	mov	rsp, rbx
