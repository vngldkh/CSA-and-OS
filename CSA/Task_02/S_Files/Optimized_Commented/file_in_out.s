...
note_random_input:
	...
	push	rbp
	push	rbx
	# Сохраняем str[] не на стеке (QWORD PTR -24[rbp]), а в регистре rbx
	mov	rbx, rdi
	sub	rsp, 8
	# В rdi записываем argv[1]
	mov	rdi, QWORD PTR 8[rsi]
	lea	rsi, .LC0[rip]
	call	fopen@PLT
	# fin продолжаем хранить в ax, не сохраняем на стек (QWORD PTR -8[rbp])
	test	rax, rax
	je	.L5
	# В di храним текущий символ, так как всегда он первый аргумент для вывода
	movsx	edi, BYTE PTR [rbx]
	mov	rbp, rax
	cmp	dil, -1
	je	.L3
	# i храним в rbx, а не на стеке (DWORD PTR -12[rbp])
	add	rbx, 1
	.p2align 4,,10
	.p2align 3
.L4:
...
.L24:
	push	r13
	push	r12
	# Первый аргмент str[] хранился в di, записываем не на стек, а в регистр r12
	mov	r12, rdi
	push	rbp
	# Второй аргумент argv хранился в si, записываем его не на стек, а в регистр bp
	mov	rbp, rsi
	push	rbx
	sub	rsp, 8
	call	rand@PLT
	...
	jle	.L16
	lea	eax, -1[rdx]
	# i хранится в r13, а не на стеке
	mov	r13, r12
	# size хранится в rbx, а не на стеке
	lea	rbx, 1[r12+rax]
	.p2align 4,,10
	.p2align 3
.L18:
	call	rand@PLT
	add	r13, 1
	# Результат не записываем на стек, продолжаем хранить в ax
	movsx	rdx, eax
	mov	ecx, eax
